// Raise every team's budget from £114m to £115m: +£1.0m to the
// bank, initialBudget stamped to 115. Idempotent — teams already at
// initialBudget=115 are skipped, so a re-run can't double-pay anyone.
...
// undo the raise). Mirrors scripts/increase-budget-114.ts.
//
  ...
  if (APPLY && changed > 0) {
    await prisma.auditLog.create({
      data: {
        ...
        details: JSON.stringify({ from: 114, to: NEW_BUDGET, raise: RAISE, teams: changed }),
      },
    });
  }
