// Raise every team's budget from £100m to £105m: +£5.0m to the bank,
// initialBudget stamped to 105. Idempotent — teams already at
// initialBudget=105 are skipped, so a re-run can't double-pay anyone.
  ...
  if (APPLY && changed > 0) {
    await prisma.auditLog.create({
      data: {
        ...
        details: JSON.stringify({ from: 100, to: NEW_BUDGET, raise: RAISE, teams: changed }),
      },
    });
  }
