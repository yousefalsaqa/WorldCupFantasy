...
async function main() {
  ...
  const league = await prisma.league.findFirst({
    where: { name: 'the bois' },
    select: { memberships: { select: { team: { select: { id: true, name: true, totalPoints: true, user: { select: { username: true } } } } } } },
  });
  const teams = league!.memberships.map(m => m.team!).sort((a,b) => b.totalPoints - a.totalPoints);
  const rows = [];
  ...
  // Rank before this stage (by total minus this stage's points) for a genuine trend arrow
