// Raise every team's budget from £108m to £109m: +£1.0m to the bank,
// initialBudget stamped to 109. Idempotent — teams already at
// initialBudget=109 are skipped, so a re-run can't double-pay anyone.
...
// undo the raise). Mirrors scripts/increase-budget-108.ts.
//
  ...
  if (APPLY && changed > 0) {
    await prisma.auditLog.create({
      data: {
        ...
        details: JSON.stringify({ from: 108, to: NEW_BUDGET, raise: RAISE, teams: changed }),
      },
    });
  }
